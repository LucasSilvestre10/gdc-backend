...
export class EmployeeRepository {
  ...
  async list(
    ...
    if (filter.isActive === "all") {
      ...
      !Object.prototype.hasOwnProperty.call(filter, "isActive") ||
  ...
  async searchByName(query: string, filters: any = {}): Promise<Employee[]> {
    ...
    return await this.employeeModel.find(matchQuery).sort({ name: 1 }).exec();
  }
  ...
  async searchByNameOrCpf(
    ...
    const searchQuery = {
      ...
      $or: [
        ...
        ...(cpfRegexPattern
          ? [{ document: { $regex: cpfRegexPattern, $options: "i" } }]
          : []), // CPF flexível se aplicável
      ],
    };

    // Executa consulta paginada
    const mongooseQuery = this.employeeModel
      .find(searchQuery)
      .sort({ name: 1 })
      .skip((page - 1) * limit)
      .limit(limit);
